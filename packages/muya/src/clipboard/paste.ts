...
// inline text — plain-text editor semantics (df174e68). Returns true when it
// handled the paste, false to fall through to `applyParsedPaste`.
//
...
// the trailing newline that text/plain faithfully keeps (ea876148).
...
async function applyPaste(clipboard: Clipboard, data: IPasteData): Promise<void> {
    ...
    if (/html|text/.test(copyType)) {
        ...
        if (isLiteralAnchor || isPlainInlineSpaces) {
            ...
            // Whole-line copies (text/plain ends in a newline) pasted at a
            // collapsed caret keep their line structure instead of being
            // spliced inline (df174e68); fall through to the normal parse when
            // it doesn't apply.
